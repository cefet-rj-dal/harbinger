...
The MIT-BIH objects are separate dataset collections by lead. This notebook helps the reader see how those objects are organized before building anomaly or event-detection experiments on the ECG signals.
...
- plot the first signal together with the labeled events
...
har_plot(
  harbinger(),
  mit_mlii_info$first_series[[mit_mlii_info$plot_column]],
  event = mit_mlii_info$first_series$event
)
```
...
har_plot(
  harbinger(),
  mit_v1_info$first_series[[mit_v1_info$plot_column]],
  event = mit_v1_info$first_series$event
)
```
...
har_plot(
  harbinger(),
  mit_v2_info$first_series[[mit_v2_info$plot_column]],
  event = mit_v2_info$first_series$event
)
```
...
har_plot(
  harbinger(),
  mit_v5_info$first_series[[mit_v5_info$plot_column]],
  event = mit_v5_info$first_series$event
)
```
