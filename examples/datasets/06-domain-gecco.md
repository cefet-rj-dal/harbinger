...
This step expands the packaged `gecco` object to the full collection and reports the basic structure of the first series. The point is to confirm, on the actual object, which columns carry the signal and the event labels before any modeling choice is made.
...
This preview is the practical check for the notebook: it shows what the first `gecco` signal actually looks like and how the labels are arranged on top of it. That is what the reader needs before deciding whether the collection is suitable for anomaly or event-detection experiments.
