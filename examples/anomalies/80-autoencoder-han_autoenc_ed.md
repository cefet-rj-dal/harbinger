...
This setup anchors the notebook in the specific series used to examine `han_autoencoder(..., autoenc_ed, ...)`. The semantic point is the one stated above: autoencoder (encode-decode): An autoencoder reconstructs sliding windows; large reconstruction errors indicate anomalies, so the raw signal needs to be visible before any fitting step hides that structure behind model output.
...
This first visual pass establishes what the method should react to in the raw series. Keep the method summary in mind here, because autoencoder (encode-decode): An autoencoder reconstructs sliding windows; large reconstruction errors indicate anomalies and the plot tells you whether that structure is clean, weak, local, repeated, or mixed with other effects.
...
![plot of chunk unnamed-chunk-5](fig/80-autoencoder-han_autoenc_ed/unnamed-chunk-5-1.png)
...
The choices below turn the central modeling idea into concrete parameters. They matter because autoencoder (encode-decode): An autoencoder reconstructs sliding windows; large reconstruction errors indicate anomalies, so each argument controls how strongly the method will emphasize that pattern when it later produces reconstruction-based anomaly flags.
...
The evaluation asks whether the reconstruction-based anomaly flags produced by `han_autoencoder(..., autoenc_ed, ...)` match the labeled structure on this dataset. Read the scores as evidence about the method's assumptions in practice, not as detached summary numbers.
...
![plot of chunk unnamed-chunk-11](fig/80-autoencoder-han_autoenc_ed/unnamed-chunk-11-1.png)
...
![plot of chunk unnamed-chunk-12](fig/80-autoencoder-han_autoenc_ed/unnamed-chunk-12-1.png)
